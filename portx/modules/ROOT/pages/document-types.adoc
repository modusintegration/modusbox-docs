...
Configuring a xref:glossary#d[*_Document Type_*] for a partner means configuring settings specific to that document type.
...
. In the *CONFIGURATION* section of the left-hand navigation pane, click *Document Types*.
+
...
. On the <<img-document-types>>, click the Document Type you want to edit.
+
The <<img-document-types-edit>> appears.


[[img-document-types-edit]]

image::document-types-edit.png[img-document-types-edit, title="Existing Document Types Page"]

[start=2]
. Edit settings as appropriate. +
For information about editing specific settings, see <<Add a New Document Type>>.
...
* *Company name* (see xref:actor-information.adoc[Actor Information])
* *Standard* (see <<Format>>)
* *Version* (see <<Format>>)
...
|Select either of these checkboxes in order to use the values selected in the corresponding sections of xref:x12-settings[x12-Settings].
...
|Select this checkbox in order to use the values selected in the corresponding sections of xref:csv-settings[CSV-Settings].
...
. On the <<img-document-types>>, click *Properties*.

. Click *New Property*. +
The *Add Property* box appears.
