...
In this example of a shared routing scenario, your organization wants to receive the same xref:glossary#d[Document Type] *Y* from two different xref:glossary#p[Partners], map it to an internal standard Document Type *X*, and send it to a backend API.
