= Integration Hub Tutorial: Designing a Transaction

This tutorial:

* Illustrates the role Integration Hub plays in B2B transaction processing. 
* Introduces _Transaction Designer_, a Integration Hub feature that enables you to design the elements of a transaction from beginning to end.
...
* <<Copy Integration Hub Information>>
* <<Configure Integration Hub Routing Engine in Anypoint Studio>>
...
+
NOTE: If you are unable to access Integration Hub, see your PortX account administrator for assistance, or to setup a trial account.

* Anypoint Studio with Mule 3.9.1

=== Download Tutorial Materials

. Download link: https://drive.google.com/open?id=1ZqNUazJHoBJ5Xj25L9OXEXYRilrsdkTH[Integration Hub Tutorial Materials].
. Click *Integration Hub Tutorial Materials.zip* to expand it.
...
The Supplier is *AAZFasteners*, a fictional retailer of nuts and bolts, also identified on the xref:img-trading-partners[] as *YOUR COMPANY*.

<<<<<<< HEAD:partner-manager/modules/ROOT/pages/partner-manager-tutorial-td.adoc
NOTE: In your copy of Partner Manager, *YOUR COMPANY* is probably not named *AAZFasteners*. However, for illustration purposes, we refer to *YOUR COMPANY* as *AAZFasteners* throughout this tutorial. If you want to change the name of *YOUR COMPANY* to *AAZFasteners*, see xref:partner-configuration#create-and-configure-partners[Create and Configure Partners]. However, the tutorial will work without making this change.
...
In the tutorial, AcmeHomeSupply uses Partner Manager to order products from AAZFasteners. In the following sections, you will:
=======
NOTE: In your copy of Integration Hub, *YOUR COMPANY* is probably not named *GearCo*. However, for illustration purposes, we refer to *YOUR COMPANY* as *GearCo* throughout this tutorial. If you want to change the name of *YOUR COMPANY* to *GearCo*, see xref:partner-configuration#create-and-configure-partners[Create and Configure Partners]. However, the tutorial will work without making this change.

The other partner in this tutorial is the _Buyer_, a fictional retailer of home supplies. In <<Create the Buyer Partner>>, we create this partner, identifying it as *AcmeHomeSupply*.
In the tutorial, AcmeHomeSupply uses Integration Hub to order products from GearCo. In the following sections, you will:
>>>>>>> 0f39bfe... global change to integration hub:integration-hub/modules/ROOT/pages/partner-manager-tutorial-td.adoc
...
Integration Hub provides for the uploading of certificates to verify the identity of a partner.
...
Integration Hub enables you to categorize and configure specific _Document Types_. In this section, you create a Source Document Type for the Target Channel.
...
In Integration Hub, an endpoint defines the protocol, address, and other details specific to an exchange of messages between partners.
...
. Click *Save Template*.
+
The <<img-td-save-transaction-AAZFasteners>> appears.


[[img-td-save-transaction-AAZFasteners]]

image::yc/td-save-transaction-AAZFasteners.png[img-td-save-transaction-AAZFasteners,title="Save transaction Page"]


[start=3]

. On the <<img-td-save-transaction-AAZFasteners>>, click *Close*.
+
The <<img-td-target-channel-in-progress-AAZFasteners>> appears.
...
substituting *Acme* for *AAZFasteners*.
...
Configuring an Amazon Simple Storage Service (Amazon S3) bucket enables you to view message payloads in <<Monitor the Transaction>>. You will add configuration data from the bucket in <<Configure Integration Hub Routing Engine in Anypoint Studio>>.


== Copy Integration Hub Information

In order to identify your Integration Hub environment and its API Key to Anypoint Studio:
...
. On the <<index.adoc#img-integration-hub-start,B2B Transactions Page>>, in the left-hand navigation pane, click *Administration*, then click *Environments*.
. In the row that corresponds to the environment with which you are working, click the *+* icon to copy the value.

=== Identify or Create an API Key

In order to connect your application to Integration Hub, you must enter an xref:glossary#secta[API Key] and environment Id.

If you have an existing API Key, use it. If you do not know the API Key, consult your organization's Integration Hub administrator.

If your organization has not created an API Key, you can use Integration Hub to create one.
...
NOTE: You must uninstall the existing Integration Hub connector from Anypoint Studio before importing.
...
. Click to expand *Integration Hub Tutorial Materials*, and then click *Apps* to expand that folder.
...
=== Insert Integration Hub Environment Information
...
image::tpm-executed.png[img-tpm-executed, title="Integration Hub Executed"]
...
. Click *Send*. 
. Go to <<Monitor the Transaction>> to see how Integration Hub displays transaction information. 
...
Integration Hub enables you to view a transmission payload. For example, in <<img-transaction-monitoring-successful-transaction>>, click the Payload icon in the right pane (identified in the figure by the gold arrow). The <<img-transmission-payload>> appears, presenting payload data.
