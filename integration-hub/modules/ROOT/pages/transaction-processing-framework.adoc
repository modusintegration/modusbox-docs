...
* xref:schema.adoc[Schema] to use for validation
* xref:map.adoc[Map] to transform the message into the required target format
...
=== Validate
Makes sure the required data is present in the required format. This validation is performed dynamically using a xref:schema.adoc[Schema] retrieved from the xref:tpmapi[TPM API].
