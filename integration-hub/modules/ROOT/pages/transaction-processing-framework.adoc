...
xref:glossary#sectb[_PortX Transaction Processing Framework_] enables developers to build a single set of 
ifdef::mule[]
Mule Runtime
endif::[]
...
This page identifies high-level framework components and their roles in end-to-end transaction processing.
...
PortX provides a Routing Engine engineered to meet the needs of most customer scenarios. We also allow for extensibility where needed...
