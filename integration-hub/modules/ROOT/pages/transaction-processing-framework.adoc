...
Portx Integration Hub includes a  xref:glossary#sectb[Transaction-processing solution] which serves as a routing and translation layer between your internal xref:glossary#a[Application]s
...
== Routing Engine
A key feature of the xref:glossary#sectb[Transaction-Processing Framework] is the Integration Hub xref:glossary#sectr[Routing Engine]. 
...
The xref:glossary#sectb[Routing Engine] processes a wide range of transmissions from different partners, dynamically applying rules and configuration data stored in xref:index.adoc[Integration Hub].
application that can process a wide range of messages from many different partners by dynamically applying rules and configuration data stored in xref:index.adoc[Integration Hub].
...
The integration Hub Connector runs inside an ESB and coordinates all interactions that ESB has with PortX APIs' TPM and Tracking.
