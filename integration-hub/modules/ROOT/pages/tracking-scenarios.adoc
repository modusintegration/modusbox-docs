...
This section identifies scenarios in which Integration Hub (APM) is useful to track B2B exchanges. A basic knowledge of APM is assumed; see xref:anypoint-integration-hub[Integration Hub] for more information.


== Replaying an Incomplete Transaction

On the xref:monitor-b2b-transactions[B2B Transactions Page], the Transactions View includes a table in which each row corresponds to a transaction.  The first column shows transaction completion status, as follows:
...
For more information, see "Transactions View" inxref:transaction-monitoring[Transaction Monitoring].
...
The *Dashboard* at the top of the xref:transaction-monitoring[Transaction Monitoring] provides a summary of what has happened in a given timeframe (the default is the current day) including how many errors have been detected during that timeframe.
...
. On *View Dropdown List* of the xref:monitor-b2b-transactions[B2B Transactions Page], choose *Transmissions*.
+
...
. On *View Dropdown List* of the xref:monitor-b2b-transactions[B2B Transactions Page], choose *Documents*.
+
...
== Viewing Multiple Payloads

. On the xref:transaction-monitoring#img-transaction-monitoring[Transaction Monitoring Page], identify the transaction for which you want to view multiple payloads, as in <<img-tm-start>>. 

[[img-tm-start]]

image::tm-start-arrow.png[img-tm-start, title="Transaction Monitoring Page, Filtered"]

[step=2]

. On the <<img-tm-start>>, click the transaction for which you want to view payloads. +
The Business Documents and Transmission that comprise the transaction appear. 


[[img-tm-row]]

image::tm-row.png[img-tm-row, title="Transaction Monitoring Page, Row Detail"]

[start=3]

. Select an element for which you want to view the payload.
+ 
The element (in this case, an 850 document) appears.

[[img-tm-850]]

image::tm-850-arrow.png[img-tm-850, title="Transaction Monitoring Page, 850"]

[start=4] 

. In the Document Detail Pane of the <<img-tm-850>>, in the Message Payload section, click the right-facing arrow. 
+
The Payload Detail page appears. 

[[img-tm-payload]]

image::tm-payload.png[img-tm-payload, title="Transaction Monitoring Page, Payload"]

[start=5]

. At the bottom of the page, click *Minimize*.
+
The <<img-tm-row>> appears. Note that a tab representing the minimized payload appears in the bottom right corner of the page. 

[[img-tm-payload-minimized]]

image::tm-payload-minimized.png[img-tm-payload-minimized, title="Transaction Monitoring Page, Minimized"]

[start=6]

. To view another payload, repeat Steps 2-5. 
+
. To view the minimized payload, click the payload tab. 
