...
|Uses Reusable Endpoint checkbox

|It is possible to create reusable configuration for endpoints and use them instead of entering the endpoint connection details every time. 

This feature is available for all Receive type endpoints except *AS2* endpoint.
For more details on using Reusable config see <<Using Reusable Config>>
...
== Using Reusable Config

. On the xref:img-endpoint-add[] select *Using Reusable Config* checkbox. 
. The page gets updated and a new Button and Box appears. 

[[img-endpoint-using-reusable-config]]
image::endpoint-using-reusable-config.png[img-endpoint-using-reusable-config, title="Endpoint Using Reusable Config"]

[start=3]
. Click on the *New Reusable Endpoint Configuration* button, xref:img-create-reusable-config[] appears appears. 

[[img-create-reusable-config, Create Reusable Configuration]]

image::reusable-configuration-page.png[img-create-reusable-config, title="Create New Reusable Configuration"]
[start=4]
. Scroll down and in the *Operation Settings* section provide values for *Host* and *Port* fields. 

NOTE: The xref:img-create-reusable-config[] page will have different fields based on Endpoint Type. The Step 4 above is for HTTP endpoint.
