...
PortX enables you to create, populate and edit a 
_xref:glossary.adoc#sectl[lookup table]_. A lookup table is a table that facilitates transaction processing by associating values. For example, this page shows how to create a lookup table that associates internal partner product codes with customer product codes.
...
This section shows you how to use the PortX user interface to create a lookup table. Alternatively, you can create a lookup table through calls to the xref:partner-manager-apis.adoc[Partner Manager APIs]; see xref:lookup-tables-programmatic.adoc[Create a Lookup Table]. See xref:lookup-tables-programmatic.adoc[Create a Lookup Table] also for instructions for querying a table.
