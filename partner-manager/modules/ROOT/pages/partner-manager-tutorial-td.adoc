...
You can leave the *Certificate purpose* to default value, which is *Any*
...
. Monitor the console for few seconds, it should start polling PM and load the dynamic endpoints. Once it finishes it prints *TPM manager Executed* in the console. 

[[img-tpm-executed]]

image::tpm-executed.png[img-tpm-executed, title="Partner Manager Executed"]

Now proceed to <<Send a Message>>. If Polling doesn't start automatically, then follow <<Start Application in debug mode>> 
