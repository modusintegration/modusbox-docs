...
* Illustrates the role Partner Manager plays in B2B transaction processing. 
* Introduces _Transaction Designer_, a Partner Manager feature that enables you to design the elements of a transaction from beginning to end.
* Provides instructions for using Transaction Designer to configure Partner Manager so that you can simulate a transaction and see how Partner Manager processes it.
...
* <<Copy Partner Manager Information>>
* <<Configure Partner Manager Routing Engine in Anypoint Studio>>
...
* Anypoint Platform user account with Partner Manager access
+
NOTE: If you are unable to access Partner Manager, see your Mulesoft account administrator for assistance.

* Anypoint Studio with Mule 3.9.1

=== Download Tutorial Materials

. Download link: https://drive.google.com/open?id=1ZqNUazJHoBJ5Xj25L9OXEXYRilrsdkTH[Partner Manager Tutorial Materials].
. Click *Partner Manager Tutorial Materials.zip* to expand it.

== Get Started
. xref:index.adoc#start-partner-manager[Start Partner Manager].
If you are logging in for the first time, you will see a pop up window in which you will be prompted to select *Company* and *Business Group*. 
...
The Supplier is *GearCo*, a fictional retailer of nuts and bolts, also identified on the xref:img-partner-configuration[] as *YOUR COMPANY*.

NOTE: In your copy of Partner Manager, *YOUR COMPANY* is probably not named *GearCo*. However, for illustration purposes, we refer to *YOUR COMPANY* as *GearCo* throughout this tutorial. If you want to change the name of *YOUR COMPANY* to *GearCo*, see xref:partner-configuration#create-and-configure-partners[Create and Configure Partners]. However, the tutorial will work without making this change.
...
In the tutorial, AcmeHomeSupply uses Partner Manager to order products from GearCo. In the following sections, you will:
...
include::_partials/tutorial/supplier/identifier-x12-isa-supplier.adoc[]

==== Add Supplier AS2 Identifier

include::_partials/tutorial/supplier/identifier-as2-supplier.adoc[]

=== Upload Certificates

Partner Manager provides for the uploading of certificates to verify the identity of a partner.

==== Upload the Supplier Certificate

include::_partials/tutorial/supplier/certificate-supplier.adoc[]
...
include::_partials/tutorial/supplier/start-td-aaz.adoc[]

=== Create the Target Channel

include::_partials/tutorial/supplier/td-target-channel-aaz.adoc[]
...
Partner Manager enables you to categorize and configure specific _Document Types_. In this section, you create a Source Document Type for the Target Channel.

==== To Create the Source Document Type for the Target Channel

include::_partials/tutorial/supplier/td-target-channel-source-doc-aaz.adoc[]


=== Configure Endpoints

An _endpoint_ is the entry point to a service, a process, or a queue or topic destination in service-oriented architecture.
In Partner Manager, an endpoint defines the protocol, address, and other details specific to an exchange of messages between partners.

==== Configure the Target Endpoint

include::_partials/tutorial/supplier/td-target-channel-endpoint-aaz.adoc[]


=== Save the Transaction

. In the *Transaction Name* box, type *TargetPurchaseOrderTemplate*.
. Click *Save Template*.
+
The <<img-td-save-transaction-gearco>> appears.


[[img-td-save-transaction-gearco]]

image::yc/td-save-transaction-gearco.png[img-td-save-transaction-gearco,title="Save transaction Page"]


[start=3]

. On the <<img-td-save-transaction-gearco>>, click *Close*.
+
...
include::_partials/tutorial/buyer/create-buyer-partner.adoc[]


=== Add Buyer Identifiers

NOTE: Make sure you are configuring *AcmeHomeSupply*, not *GearCo*.

==== Add Buyer X12-ISA Identifier

To add this identifier, perform steps in <<Add Supplier X12-ISA Identifier>>,
substituting *Acme* for *GearCo*.
...
substituting *partner.pem* for *HomeOrg.p12*.

 For *Certificate purpose*, accept default value (*Any*).
...
include::_partials/tutorial/buyer/start-td-acme.adoc[]

=== Create the Source Channel


include::_partials/tutorial/buyer/td-source-channel-acme.adoc[]


=== Create the Source Document

include::_partials/tutorial/buyer/td-source-channel-source-doc-acme.adoc[]


=== Create the Buyer Receive Endpoint

include::_partials/tutorial/buyer/td-source-channel-endpoint-acme.adoc[]
...
[start=2]
. In the *Property Type* listbox, click the down-arrow.
. In the drop-down list, scroll down to select *Transaction Set ID (system)*.
. In the *Property Source Type* box, select *Constant*.
. In the *Path* box, type *850*.
. Click *Save Property*.

===== ServicePath

. This property is required only if selected *HTTP endpoint* in URL Type. 
+
On the <<img-td-linked-endpoint-properties-acme>>, click *New Property*.
+
The Add Property Page appears.
...
[start=2]
. In the *Property Type* listbox, click the down-arrow. 
. In the drop-down list, scroll down to select *ServicePath*. 
+
If this property is not available in list, see xref:property-type.adoc#to-create-a-new-property-type[Create New Property Type] to create a new Property with name *ServicePath*
. In the *Property Source Type* box, select *Constant*.
. In the *Path* box, type *edi*.
. Click *Save Property*.
...
. On the Pick a document Page, click the row that corresponds to the Target Document Type - that is, the row for which the value in the *Name* column is  *JSON-PurchaseOrder(GearCo)*.
+
The <<img-td-new-map-acme>> appears, with *JSON-PurchaseOrder(GearCo)* in the *Target Document* box.
. In the *Mapping Type* box, click *DataWeave*.
+
For more information, see xref:https://docs.mulesoft.com/mule-runtime/3.8/dataweave-reference-documentation[DataWeave].
. Click *Save*.
+
...
. Click *Save* at top right side to save the transaction.
...
Configuring an Amazon Simple Storage Service (Amazon S3) bucket enables you to view message payloads in <<Monitor the Transaction>>. You will add configuration data from the bucket in <<Configure Partner Manager Routing Engine in Anypoint Studio>>.


== Copy Partner Manager Information

In order to identify your Partner Manager environment and its API Key to Anypoint Studio:
...
. On the <<index.adoc#img-partner-manager-start,B2B Transactions Page>>, in the left-hand navigation pane, click *Administration*, then click *Environments*.
. In the row that corresponds to the environment with which you are working, click the *+* icon to copy the value.

=== Identify or Create an API Key

In order to connect your application to Partner Manager, you must enter an xref:glossary#secta[API Key] and environment Id.

If you have an existing API Key, use it. If you do not know the API Key, consult your organization's Partner Manager administrator.

If your organization has not created an API Key, you can use Partner Manager to create one.

WARNING: The API Key is used by every Mule application across your entire Master xref:access-management/organization[Organization] that communicates with Partner Manager. Therefore, before you create a new API Key, coordinate with your organization's Partner Manager administrator to ensure that none of your organization's processes are using an existing API Key because, if they are, creating a new API Key will cause them to cease functioning. In that case, instead of creating a new API Key, use the existing API Key.
...
NOTE: You must uninstall the existing Partner Manager connector from Anypoint Studio before importing.
...
. Click to expand *Partner Manager Tutorial Materials*, and then click *Apps* to expand that folder.
. In the *Apps* folder, click *apm-routing-engine.zip*, then click *Open*. +
...
=== Insert Partner Manager Environment Information

. In Anypoint project (expand if necessary), expand the *src/main/resources*, double-click *apm.prod.edi.properties*. +
Project properties appear in the Anypoint Studio Canvas.
. In Line 1, replace *{Insert environment id}* with the environment ID you copied in <<Copy your Environment ID>>.
. In Line 2, replace *{Insert API Key}* with the API Key
...
. Click *Run*.
. Verify the application is deployed successfully on Anypoint Studio console, 
  
[[img-app-deploy-verify]]

image::app-deploy-verify.png[img-app-deploy-verify, title="Verify Application Deployed"]

[start = 7]

. Monitor the console for a few seconds; it should begin polling PM and loading the dynamic endpoints. On completion, it prints *TPM manager Executed* in the console. 

[[img-tpm-executed]]

image::tpm-executed.png[img-tpm-executed, title="Partner Manager Executed"]

Proceed to <<Send a Message>>. If Polling does not start automatically, follow <<Start Application in debug mode>>.
...
. Click *Send*. 
. Go to <<Monitor the Transaction>> to see how Partner Manager displays transaction information. 
...
Partner Manager enables you to view a transmission payload. For example, in <<img-transaction-monitoring-successful-transaction>>, click the Payload icon in the right pane (identified in the figure by the gold arrow). The <<img-transmission-payload>> appears, presenting payload data.
...
include::_partials/tutorial/troubleshooting.adoc[]
