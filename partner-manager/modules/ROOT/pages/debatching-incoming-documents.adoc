...
Purchase orders (POs) sent in XML are typically enclosed in a GS envelope, which is in turn enclosed in an GS envelope. In order to process the POs, they must be _debatched_: that is, separated so that each PO is enclosed in its own GSA and ISA envelopes, as shown in <<img-xml-debatching>>.
