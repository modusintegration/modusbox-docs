...
Each incoming transaction from a partner or internal system is delivered through a transport protocol xref:portx/endpoints[_endpoint_] configured to receive the message.
...
* xref:portx/endpoints[Endpoint] to which the transformed message is to be delivered
...
* xref:portx/partner-configuration[Configure how B2B transactions are processed]
* xref:portx/transaction-monitoring[Monitor and manage processed transactions]
...
The Partner Manager Connector (PMC) runs inside xref:mule-user-guide[Mule Runtime] and coordinates all interaction with the PortX APIs (TPM and Tracking).
...
* Does any necessary data translation, such as resolving partner values to your company's values using xref:portx/[lookup tables] functions, and flows.
...
For information about how endpoints are created, see xref:portx/resolution-processes#endpoint-resolution[Endpoint Resolution].
...
=== Document Type
xref:portx/document-types[Document Types] are configured in PortX. A given document type
...
The PortX user interface enables xref:portx/partner-configuration[partner configuration], xref:portx/administration[administration] and detailed xref:portx/transaction-monitoring[transaction monitoring].
