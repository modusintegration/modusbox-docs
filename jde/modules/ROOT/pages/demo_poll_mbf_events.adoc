...
The *mule-arifact.json* file needs to be updated per environment as follows.
...
To redirect the _JD Edwards EntrpriseOne™_ Logger to Mule Logger (allowing you to see the JDE activity in both Console and JDE files defined in the *jdelog.properties*, you may add the following *Async Loggers* to *log4j2.xml* file.
...
If transactions are retrieved (*When* condition is true) we need to iterate over all transactions that have been retrieved.
