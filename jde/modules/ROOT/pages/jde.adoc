...
. In a web browser, go to https://modusbox.atlassian.net/servicedesk/customer 
. Log into you support user account (see the _Creating a support user_ section above)
...
===== Requests
* Global
** Instances Detail: http://{sm-host}:{sm-port}/manage/mgmtrestservice/instancesinfo
* Enterprise Server instance (Required for each Enterprise Server instance)
** Configuration Summary by instance: http://{sm-host}:{sm-port}/manage/mgmtrestservice/configsummary?instanceName={instance}
** Ini Configuration by Instance: http://{sm-host}:{sm-port}/manage/mgmtrestservice/iniconfig?instanceName={instance}
...
. Under the SCFHA folder/directory, navigate to targets/{web-instance-name}/config/
. Attach the following files to us for review (rename them to {filename}_server.ini as not to conflict with the files in your Mule project)
* jdbj.ini (rename to _jdbj_server.ini_)
* jdeinterop.ini (rename to _jdeinterop_server.ini_)
* tnsnames.ora (if using an Oracle database)(rename to _jdbj_server.ini_)
...
eg. /u01/Oracle/SCFHA/jde_home/targets/{instanceName}/config
